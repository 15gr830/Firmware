function [xa_apo,Pa_apo,Rot_matrix,eulerAngles,debugOutput,q]...
    = AttitudeEKF2grav(approx_prediction,use_inertia_matrix,zFlag,dt,z,q_rotSpeed,q_rotAcc,q_acc,q_mag,r_gyro,r_accel,r_ptam,J)
...
%% update
%% update
if zFlag(1)==1&&zFlag(2)==1&&zFlag(3)==1&&zFlag(4)==1
    ...
    R_v=[r_gyro,r_gyro,r_gyro,r_accel,r_accel,r_accel,r_ptam(1),r_ptam(1),r_ptam(1),r_ptam(1),r_ptam(1),r_ptam(1)];
    %observation matrix
    %[zw;ze;zmk];
    H_k=[  E,     Z,      Z,    Z; 
           Z,     Z,      E,    Z;
    ...
    y_k=z(1:12)-H_k*x_apr;
    
    
    %S_k=H_k*P_apr*H_k'+R;
    S_k=H_k*P_apr*H_k';
    S_k(1:12+1:end) = S_k(1:12+1:end) + R_v;
    K_k=(P_apr*H_k'/(S_k));
    ...
    if zFlag(1)==1&&zFlag(2)==1&&zFlag(3)==1
        ...
        R_v=[r_gyro,r_gyro,r_gyro,r_accel,r_accel,r_accel,r_ptam(1),r_ptam(1),r_ptam(1)];
        %observation matrix
        %[zw;ze;zmk];
        H_k=[  E,     Z,      Z,    Z;
            Z,     Z,      E,    Z;
        ...
        if zFlag(1)==1&&zFlag(2)==0&&zFlag(3)==0
            ...
            if  zFlag(1)==1&&zFlag(2)==1&&zFlag(3)==0
                ...
                if  zFlag(1)==1&&zFlag(2)==0&&zFlag(3)==1
                    ...
                    %                     0,0,0,0,r_mag,0;
                    %                     0,0,0,0,0,r_mag];
                    R_v=[r_gyro,r_gyro,r_gyro,r_ptam(1),r_ptam(1),r_ptam(1)];
                    %observation matrix
...
%making a quaternion
q0 = [0.5*sqrt(Rot_matrix(1,1)+Rot_matrix(2,2)+Rot_matrix(3,3)+1)];
q = [q0;...
    sign(q0)*((Rot_matrix(2,3)-Rot_matrix(3,2))/4*q0);...
     sign(q0)*((Rot_matrix(3,1)-Rot_matrix(1,3))/4*q0);...
     sign(q0)*((Rot_matrix(1,2)-Rot_matrix(2,1))/4*q0)];
